...
class qe_wannier_in:
    def __init__(self, cif_file, toml_file, so, mag):
        ...
        self.info_wannier = info["wannier"]
    ...
    def convert2dos(self):
        """
        You MUST call this function after convert2band() was called.
        """
        self.control_str = self.control_str.replace("'bands'", "'nscf'")
        self.system_str = self.system_str.replace("'smearing'", "'tetrahedra'") 
        self.electrons_str = re.sub("  conv_thr.*\n", "  conv_thr = 1.d-10\n", self.electrons_str)
        self.kpoints_str = "K_POINTS {automatic}\n"
        kmesh_high = [ min(3*grid, 40) for grid in self.kmesh]
        self.kpoints_str += "{0[0]} {0[1]} {0[2]}  0 0 0\n".format(kmesh_high)
    ...
    def write_wannier(self, wannier_in):
        ...
        with open(wannier_in, "w") as fp:
            ...
            if "dis_num_iter" in info.keys():
              fp.write(f"dis_num_iter = {info['dis_num_iter']}\n")
            else:
    ...
    def write_proj(self, proj_in, is_kresolved, use_smearing):
        with open(proj_in, "w") as fp:
            ...
            fp.write(" outdir = './work'\n")
            if is_kresolved:
                fp.write(" kresolveddos = .true.\n")
            else:
                fp.write(" kresolveddos = .false.\n")
            if use_smearing:
                fp.write(" degauss = {:0.3f}\n".format(self.degauss))
            fp.write(" DeltaE = 0.01\n")
            fp.write(" Emax = \n")
            fp.write(" Emin = \n")
            fp.write("/\n")

    def write_dos(self, proj_in, use_smearing):
        with open(proj_in, "w") as fp:
            fp.write("! generated by {}\n".format(__file__.split("/")[-1]))
            fp.write("&dos\n")
            fp.write(" prefix = 'pwscf'\n")
...
if __name__ == "__main__":
    ...
    qe_wan.write_proj("band/proj_fat.in", is_kresolved=True, use_smearing=True)
    qe_wan.write_pp("band/pp.in")

    if not os.path.exists("dos"): os.mkdir("dos")
    qe_wan.convert2dos()
    qe_wan.write_pwscf_in("dos/nscf.in")
    qe_wan.write_proj("dos/proj.in", is_kresolved=False, use_smearing=False)
    qe_wan.write_dos("dos/dos.in", use_smearing=False)

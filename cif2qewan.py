...
class qe_wannier_in:
    ...
    def convert2dos(self):
        ...
        self.electrons_str = re.sub("  conv_thr.*\n", "  conv_thr = 1.d-10\n", self.electrons_str)
        self.kpoints_str = "K_POINTS {automatic}\n"
        kmesh_high = [ 3*grid for grid in self.kmesh]
    ...
    def write_proj(self, proj_in, is_kresolved):
...
if __name__ == "__main__":
    ...
    qe_wan.write_proj("band/proj_fat.in", is_kresolved=True)
    qe_wan.write_pp("band/pp.in")

    if not os.path.exists("dos"): os.mkdir("dos")
    qe_wan.convert2dos()
    qe_wan.write_pwscf_in("dos/nscf.in")
    qe_wan.write_proj("dos/proj.in", is_kresolved=False)
